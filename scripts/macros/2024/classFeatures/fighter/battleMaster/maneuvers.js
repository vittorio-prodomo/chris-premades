...
async function useRiposte({workflow}) {
    ...
    // ⚠️ `maneuverAttack` marks this as an attack GENERATED BY a maneuver, so the superiority
    // driver does not offer a second maneuver on it — RAW, "You can use only one maneuver per
    // attack" (2024 Combat Superiority, verbatim). Riposte is itself the maneuver on this attack.
    await workflowUtils.syntheticItemRoll(selected, [workflow.targets.first()], {
        options: {workflowOptions: {maneuverAttack: true}}
    });
...
export let maneuversRiposte = {
    ...
    // 1.0.2 — T83 follow-ups: auto-offer on a miss, and stop double-spending the die.
